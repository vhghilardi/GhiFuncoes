...
uses System.SysUtils,
  ...
  {$ENDIF ANDROID}
    ...
    function API_Login(pLogin, pPass: String): String;
    ...
    function getCamposJsonString(json, value: String): String;
          ...
          procedure ProgressBarTime(pTela : TForm;
                                    pTempo : Integer;
                                    pPosicao : TAlignLayout = TAlignLayout.Center;
                                    pCor : TAlphaColor = TAlphaColorRec.Lightblue);
          procedure ProgressBarReg(pTela: TForm;
                                   pQtdRegistros : Integer;
                                   pPosicao: TAlignLayout;
                                   pCor: TAlphaColor);
...
var
   ...
   vRetProgressBarPAI, vRetProgressBar : TRectangle;
   vLabel : TLabel;
...
procedure TUtil.ProgressBarTime(pTela: TForm; pPosicao: TAlignLayout;
  pCor: TAlphaColor);
begin

end;

procedure TUtil.ProgressBarReg(pTela: TForm; pPosicao: TAlignLayout;
  pCor: TAlphaColor);
begin
//vRetProgressBarPAI, vRetProgressBar
     vRetProgressBarPAI                := TRectangle.Create(nil);
     vRetProgressBarPAI.Parent         := pTela;
     vRetProgressBarPAI.Align          := pPosicao;
     vRetProgressBarPAI.Margins.Left   := 0;
     vRetProgressBarPAI.Margins.Top    := 0;
     vRetProgressBarPAI.Margins.Bottom := 0;
     vRetProgressBarPAI.Margins.Right  := 0;
     vRetProgressBarPAI.Fill.Color     := pCor;
     vRetProgressBarPAI.Stroke.Color   := pCor;
     vRetProgressBarPAI.Width          := pTela.Width - 30;
     vRetProgressBarPAI.Height         := 20;

     vRetProgressBar                := TRectangle.Create(nil);
     vRetProgressBar.Parent         := vRetProgressBarPAI;
     vRetProgressBar.Align          := TAlignLayout.Left;
     vRetProgressBar.Margins.Left   := 0;
     vRetProgressBar.Margins.Top    := 0;
     vRetProgressBar.Margins.Bottom := 0;
     vRetProgressBar.Margins.Right  := 0;
     vRetProgressBar.Fill.Color     := TAlphaColorRec.Greenyellow;
     vRetProgressBar.Stroke.Color   := TAlphaColorRec.Greenyellow;
     vRetProgressBar.Width          := 10;
     vRetProgressBar.Height         := 20;
...
{         else
         begin
             vValor := '0';
             vValor := Copy(vEdtValor.Text, 4, Length(vEdtValor.Text));
             vValor := StringReplace(vValor, '.', '', [rfReplaceAll]);

             if (vValor <> '') and (vValor <> '0') then
             begin
                  vEdtValor.Text := 'R$ ' + FormatCurr('0.,00', StrToCurr(vValor));
             end;
         end;}
     end;
end;
...
procedure TUtil.AppOnExcept(Sender: TObject; E: Exception);
var
...
begin
     ...
     DM.qryExec.SQL.Add('select ban_url_cartaocred_sandbox, ban_url_cartaocred_prod, ban_ambiente from bancard');
     DM.qryExec.Open;
     if DM.qryExec.FieldByName('ban_ambiente').AsString = 'Producao' then
     begin
          vUrlExecTrans := DM.qryExec.FieldByName('ban_url_cartaocred_prod').AsString;
     end
     else
     if DM.qryExec.FieldByName('ban_ambiente').AsString = 'Teste' then
     begin
          vUrlExecTrans := DM.qryExec.FieldByName('ban_url_cartaocred_sandbox').AsString;
     end;
     vMerchantId   := '40f0a893-b89c-4f43-9439-71bf39536bb5';
     vMerchantKey  := 'JCpwRSqOui6bjp8rzgYkje4Hlm3IkfP4Hdm3Q8K9';
end;
...
begin
    try
      try
          vArquivo_Ini := TIniFile.Create(System.IOUtils.TPath.Combine(System.IOUtils.TPath.GetDocumentsPath, pNomeArq + '.Config.ini'));
...
begin
     try
         ...
         vURL := vURL + '?client_id=' + URIEncode('seuid');
...
begin
    vJSON := '{"Customer":{ "CliId":' + pCliId + ', "Name":"' + pNome + '"' +
             ...
             '"SoftDescriptor":"Bancard", "CreditCard":{"CardNumber":"'+ pNumCartao + '",' +
...
function TUtil.API_Login(pLogin, pPass : String) : String;
var
  JsonStreamEnvio, JsonStreamRetorno: TStringStream;
  vIdHttp : TIdHTTP;
  vIdSSLIOHandlerSocketOpenSSL : TIdSSLIOHandlerSocketOpenSSL;
  ...
  vRetorno : TStringStream;
begin
    vJSON :='{"phone": ' + pLogin + ', "password": ' + pPass + '}';
    JsonStreamEnvio   := TStringStream.Create(vJSON);
    JsonStreamRetorno := TStringStream.Create('');
    vIdHttp           := TIdHTTP.Create();
...
{    vIdHttp.Request.Clear;
    ...
    vIdHttp.Request.CustomHeaders.AddValue('Client-Service', 'teste');
    vIdHttp.Request.CustomHeaders.AddValue('Auth-Key', 'teste');
...
//https://bancard-shellscript.c9users.io/api/auth
